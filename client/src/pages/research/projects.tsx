...
export default function ResearchProjects() {
  // Fetch ongoing and completed projects from the Django API
  const { data: ongoing, loading: loadingOngoing, error: errorOngoing } = useApi("http://127.0.0.1:8000/api/projects/?is_active=true");
  const { data: completed, loading: loadingCompleted, error: errorCompleted } = useApi("http://127.0.0.1:8000/api/projects/?is_active=false");
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto py-12 px-4">
        ...
        <div className="mb-14 text-center">
          ...
          {loadingOngoing ? (
            ...
            <p className="text-red-600">{errorOngoing}</p>
          ) : !ongoing || !ongoing.results || ongoing.results.length === 0 ? (
            <p className="text-gray-500">No ongoing projects at the moment.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {ongoing.results.map((project: any) => (
                <div key={project.id} className="bg-white rounded-2xl shadow-md p-6 border border-sky-100 flex flex-col transition-all duration-300 hover:scale-105 hover:shadow-xl hover:border-sky-300">
                  <img
                    src={project.image || "/building-placeholder.jpg"}
                    alt={project.title}
        ...
        <div className="mb-10 text-center">
          ...
          {loadingCompleted ? (
            ...
            <p className="text-red-600">{errorCompleted}</p>
          ) : !completed || !completed.results || completed.results.length === 0 ? (
            <p className="text-gray-500">No completed projects yet.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {completed.results.map((project: any) => (
                <div key={project.id} className="bg-white rounded-2xl shadow-md p-6 border border-sky-100 flex flex-col transition-all duration-300 hover:scale-105 hover:shadow-xl hover:border-sky-300">
                  <img
                    src={project.image || "/building-placeholder.jpg"}
                    alt={project.title}
        ...
        {modalProject && (
          <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-lg w-full relative border border-sky-200">
              ...
              <img
                src={modalProject.image || "/building-placeholder.jpg"}
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {modalProject.tags && modalProject.tags.map((tag: string, idx: number) => (
                  <span key={idx} className="bg-sky-200 text-sky-700 px-2 py-0.5 rounded-full text-xs">{tag}</span>
                ))}
              </div>
              <div className="text-xs text-sky-700 font-semibold mb-1">Status: {modalProject.is_active ? 'Ongoing' : 'Completed'}</div>
            </div>
          </div>
        )}
      </main>
